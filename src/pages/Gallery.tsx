...
const EventCard = ({ event }: { event: GalleryEvent }) => {
  ...
  // Reset pagination when card is collapsed
  const handleToggle = () => {
    setExpanded((prev) => {
      if (prev) setVisibleCount(PHOTOS_PER_PAGE); // reset on collapse
      return !prev;
    });
  };
                  ...
                  <div className="mt-5 flex items-center justify-center gap-3">
                    {hasMore && (
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-5 py-2 rounded-full border border-border/60 text-sm font-medium hover:bg-muted/50 transition-colors"
                      >
                        <ChevronDown className="h-4 w-4" />
                        Show {Math.min(remaining, PHOTOS_PER_PAGE)} more
                    ...
                    {visibleCount > PHOTOS_PER_PAGE && (
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-5 py-2 rounded-full border border-border/60 text-sm font-medium hover:bg-muted/50 transition-colors text-muted-foreground"
                      >
      ...
      {uploadedPhotos.length > 0 && (
        <section className="pb-16">
          <div className="container mx-auto px-4 max-w-4xl">
            ...
            {/* ── Show More / Show Less for uploaded photos ── */}
            {(() => {
                ...
                (remainingUploaded > 0 || visibleUploadedCount > PHOTOS_PER_PAGE) && (
                  <div className="mt-6 flex items-center justify-center gap-3">
                    {remainingUploaded > 0 && (
                      ...
                      >
                        <ChevronDown className="h-4 w-4" />
                        Show {Math.min(remainingUploaded, PHOTOS_PER_PAGE)} more
                        <span className="text-muted-foreground/60 text-xs">
                          ({remainingUploaded} left)
                        </span>
                      </button>
                    )}
