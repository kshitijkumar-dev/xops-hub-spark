...
const events: GalleryEvent[] = [
  ...
  // { id: "event-id", title: "Event Name", category: "Workshops", description: "...", photos: [] },
];
...
const EventCard = ({ event }: { event: GalleryEvent }) => {
  ...
  return (
    <article className="gradient-border rounded-2xl overflow-hidden card-hover animate-fade-in-up">
      <button onClick={() => setExpanded((prev) => !prev)} className="w-full text-left p-6 flex items-start justify-between gap-4 group">
        <div>
          <span className="text-xs font-semibold uppercase tracking-widest text-primary/70 mb-2 block">{event.category}</span>
          <h3 className="text-xl font-bold text-foreground group-hover:text-primary transition-colors">{event.title}</h3>
          <p className="text-sm text-muted-foreground mt-1">{event.description}</p>
          <p className="text-xs text-muted-foreground/60 mt-3">{event.photos.length > 0 ? `${event.photos.length} photos` : "Photos coming soon"}</p>
        </div>
          ...
          {event.photos.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {event.photos.map((photo) => (
                <div key={photo.id} className="relative rounded-xl overflow-hidden bg-card aspect-square group/photo">
                  <img src={photo.url} alt={photo.caption || event.title} className="w-full h-full object-cover transition-transform duration-300 group-hover/photo:scale-105" loading="lazy" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                  {photo.caption && (
      ...
      <section className="py-20 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl mx-auto text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-4"><span className="text-gradient">Gallery</span></h1>
            <p className="text-muted-foreground text-lg">Moments captured from our journey</p>
      ...
      <section className="pb-8">
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap justify-center gap-3">
            {FILTERS.map((filter) => (
              <button key={filter} onClick={() => setActiveFilter(filter)} className={`px-5 py-2 rounded-full text-sm font-medium border transition-all duration-200 ${activeFilter === filter ? "bg-gradient-to-r from-blue-500 to-purple-600 text-white border-transparent shadow-lg shadow-purple-500/20" : "bg-transparent border-border/50 text-muted-foreground hover:border-primary/50 hover:text-foreground"}`}>
                {filter}
              </button>
            ))}
          </div>
      ...
      <section className="pb-24">
        <div className="container mx-auto px-4 max-w-5xl">
          {filtered.length === 0 ? (
            ...
            <div className="flex flex-col gap-4">{filtered.map((event) => <EventCard key={event.id} event={event} />)}</div>
          )}
        </div>
      </section>
    </Layout>
